package com.handge.housingfund.common.service.collection.model.deposit;
import javax.xml.bind.annotation.XmlAccessType;
import javax.xml.bind.annotation.XmlAccessorType;
import javax.xml.bind.annotation.XmlRootElement;
import java.io.Serializable;
import java.util.ArrayList;



@XmlRootElement(name = "GetUnitPayWrongRes")
@XmlAccessorType(XmlAccessType.FIELD)
public class GetUnitPayWrongRes  implements Serializable{

   private  GetUnitPayWrongResCJEHJ    CJEHJ;  //错缴额合计信息 

   private  String    JBRZJLX;  //经办人证件类型 

   private  ArrayList<GetUnitPayWrongResGZXX>    GZXX;  //更正信息 

   private  String    JBRXM;  //经办人姓名 

   private  String    SKYHMC;  //收款银行名称

   private  String    SKYHDM;  //收款银行代码

   private  String JBRZJHM;  //经办人证件号码

   private  String    YWWD;  //业务网点 

   private  String    SKYHHM;  //收款银行户名

   private  String    DWMC;  //单位名称

   private  String    DWZH;  //单位账号 

   private  String    CZY;  //操作员 

   private  String    SKYHZH;  //收款银行账号

    private  String   JCGZNY;  //缴存更正年月

    private String BLZL;

    public String getJCGZNY() {
        return JCGZNY;
    }

    public void setJCGZNY(String JCGZNY) {
        this.JCGZNY = JCGZNY;
    }

   public  GetUnitPayWrongResCJEHJ getCJEHJ(){

       return this.CJEHJ;

   }


   public  void setCJEHJ(GetUnitPayWrongResCJEHJ  CJEHJ){ 

       this.CJEHJ = CJEHJ;   

   }

    public String getSKYHDM() {
        return SKYHDM;
    }

    public void setSKYHDM(String SKYHDM) {
        this.SKYHDM = SKYHDM;
    }

    public  String getJBRZJLX(){

       return this.JBRZJLX;   

   }


   public  void setJBRZJLX(String  JBRZJLX){ 

       this.JBRZJLX = JBRZJLX;   

   }


   public  ArrayList<GetUnitPayWrongResGZXX> getGZXX(){ 

       return this.GZXX;   

   }


   public  void setGZXX(ArrayList<GetUnitPayWrongResGZXX>  GZXX){ 

       this.GZXX = GZXX;   

   }


   public  String getJBRXM(){ 

       return this.JBRXM;   

   }


   public  void setJBRXM(String  JBRXM){ 

       this.JBRXM = JBRXM;   

   }


   public  String getSKYHMC(){ 

       return this.SKYHMC;   

   }


   public  void setSKYHMC(String  SKYHMC){ 

       this.SKYHMC = SKYHMC;   

   }


   public  String getJBRZJHM(){

       return this.JBRZJHM;

   }


   public  void setJBRZJHM(String JBRZJHM){

       this.JBRZJHM = JBRZJHM;

   }


   public  String getYWWD(){ 

       return this.YWWD;   

   }


   public  void setYWWD(String  YWWD){ 

       this.YWWD = YWWD;   

   }


   public  String getSKYHHM(){ 

       return this.SKYHHM;   

   }


   public  void setSKYHHM(String  SKYHHM){ 

       this.SKYHHM = SKYHHM;   

   }


   public  String getDWMC(){ 

       return this.DWMC;   

   }


   public  void setDWMC(String  DWMC){ 

       this.DWMC = DWMC;   

   }


   public  String getDWZH(){ 

       return this.DWZH;   

   }


   public  void setDWZH(String  DWZH){ 

       this.DWZH = DWZH;   

   }


   public  String getCZY(){ 

       return this.CZY;   

   }


   public  void setCZY(String  CZY){ 

       this.CZY = CZY;   

   }


   public  String getSKYHZH(){ 

       return this.SKYHZH;   

   }


   public  void setSKYHZH(String  SKYHZH){ 

       this.SKYHZH = SKYHZH;   

   }

    public String getBLZL() {
        return BLZL;
    }

    public void setBLZL(String BLZL) {
        this.BLZL = BLZL;
    }

    @Override
    public String toString() {
        return "GetUnitPayWrongRes{" +
                "CJEHJ=" + CJEHJ +
                ", JBRZJLX='" + JBRZJLX + '\'' +
                ", GZXX=" + GZXX +
                ", JBRXM='" + JBRXM + '\'' +
                ", SKYHMC='" + SKYHMC + '\'' +
                ", SKYHDM='" + SKYHDM + '\'' +
                ", JBRZJHM='" + JBRZJHM + '\'' +
                ", YWWD='" + YWWD + '\'' +
                ", SKYHHM='" + SKYHHM + '\'' +
                ", DWMC='" + DWMC + '\'' +
                ", DWZH='" + DWZH + '\'' +
                ", CZY='" + CZY + '\'' +
                ", SKYHZH='" + SKYHZH + '\'' +
                ", JCGZNY='" + JCGZNY + '\'' +
                ", BLZL='" + BLZL + '\'' +
                '}';
    }
}